...
io.sockets.on('connection',function(socket)
{
    ...
    socket.on('LoginAttempt', function(data) {
        ...
        DBconnection.connect();
        DBconnection.query('SELECT * FROM UserMst WHERE User_Id="' + loginInfo.id + '"', function(err, res, fields) {
                if(err) {
                    console.log(err);
                }
                else {
                    ...
                    else if (res[0].UserPwd != loginInfo.pw) {
                        socket.emit('LoginResult', "pw_fail");
                    }
                    else {
                        socket.emit('LoginResult', "success");
    ...
    socket.on('RegistAttempt', function(data) {
        var registInfo = JSON.parse(data);

        var date = new Date();
        var sqlQuery = 'INSERT INTO UserMst SET ?';
        var post = {User_Id: registInfo.id, User_Nm: registInfo.name, Login_Pwd: registInfo.pw, Use_chk: 1, In_User_Id: 'root', In_DateTime: date, Mod_User_Id: 'root', Mod_DateTime: date};
